...
function handleKeyDown(e) {
    ...
    // Кнопка ArrowLeft (37) - открытие/закрытие меню
...
function handleBackButton() {
    ...
    // 1. Если открыто модальное окно добавления плейлиста - закрываем его
    if (modal) {
        modal.remove();
        keyboardActive = false;
    ...
    // 2. Если открыто контекстное меню плейлиста - закрываем его
    if (contextMenuVisible) {
        hidePlaylistContextMenu();
        return;
    } 
    
    // 3. Если открыта группа каналов - возвращаемся к списку групп
    ...
    // 4. Если открыта панель (плейлисты/каналы/группы) - закрываем её
    ...
    // 5. Если открыто главное меню - закрываем его
    else if (isMenuOpen) {
        toggleMainMenu();
        return;
    }
    
    // Если ничего не открыто - ничего не делаем
}
    ...
    const savePlaylist = () => {
        ...
        if (name && url) {
            try {
                ...
                alert("Введите корректный URL плейлиста");
            }
        } else {
        ...
        if (parsedChannels.length > 0) {
            ...
            setTimeout(() => {
                ...
                if (playlistItems.length > index) {
                    playlistItems[index].focus();
                }
            }, 100);
        } else {
            alert("Не удалось распознать плейлист");
        }
    } catch (error) {
...
// Парсеры плейлистов
// Парсер M3U плейлиста с поддержкой #EXTGRP
function parseM3U(text) {
    const lines = text.split(/\r?\n/);
    const channels = [];
    let currentName = "";
    let currentGroup = "Без группы";  // Значение по умолчанию
    let currentLogo = "";
    let currentId = "";
    ...
    for (const line of lines) {
        ...
        if (line.startsWith("#EXTINF")) {
            ...
            // Извлекаем логотип если есть
            const logoMatch = line.match(/tvg-logo="([^"]*)"/i);
            currentLogo = logoMatch ? logoMatch[1] : "";
        }
        // Обработка #EXTGRP (альтернативный вариант указания группы)
        ...
        // Обработка URL канала
            ...
            // Сброс значений для следующего канала
            currentName = "";
...
function playCurrentChannel() {
            ...
            if (data.fatal) {
                switch (data.type) {
                    case Hls.ErrorTypes.NETWORK_ERROR:
                        alert("Ошибка сети при загрузке канала");
                        break;
                    case Hls.ErrorTypes.MEDIA_ERROR:
                        alert("Ошибка воспроизведения канала");
                        break;
                    default:
                        alert("Неизвестная ошибка при воспроизведении");
                }
            }
        });
    } 
    ...
    // Обновление активного состояния каналов
...
function handleGlobalKeyDown(e) {
    ...
    // Если нет активного элемента или это body - фокусируем видео
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Сброс таймера неактивности при движениях мыши и кликах
...
// Обработка колесика мыши для навигации
